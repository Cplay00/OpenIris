...
| 环节 | 功能 | 技术实现 |
|------|------|----------|
| **数据准备** | 自建数据集、标注转换、数据增强 | COCO/VOC/LabelMe 格式转换，智能划分，离线增强 |
| **模型训练** | 图形化训练、命令行训练、高精度训练 | Ultralytics + tkinter GUI，多阶段训练策略 |
| **模型导出** | 自动导出、一键部署到 APP | ONNX → NCNN 转换，自动复制到 assets |
