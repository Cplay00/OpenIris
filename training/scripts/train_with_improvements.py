...
MODEL_PRESETS = {
    ...
    'ghost': {
        ...
        'weights': 'yolo11n.pt',
        'description': 'YOLO11n + GhostConv 轻量卷积',
    },
    'enhanced': {
...
def get_model_config(preset_name, custom_yaml=None):
    ...
    if custom_yaml:
        return {
            ...
            'weights': 'yolo11n.pt',
            'description': f'鑷[?]畾涔夋[?]鍨? {custom_yaml}'
        }

    if preset_name not in MODEL_PRESETS:
        print(f"閿欒[?]: 鏈[?]煡鐨勬[?]鍨嬮[?]璁?'{preset_name}'")
...
鍙[?]敤鐨勬[?]鍨嬮[?]璁?
  ...
  se         - YOLO11n + SE 娉[?]剰鍔?(杞婚噺绾?
  cbam       - YOLO11n + CBAM 娉[?]剰鍔?(楂樼簿搴?
  ghost      - YOLO11n + GhostConv (鏋佽嚧杞婚噺)
    enhanced   - YOLO11n + CBAM + C2fCIB (增强版)
...
绀轰緥:
  python training/scripts/train_with_improvements.py --model cbam --data configs/dataset_custom.yaml
    ...
    print(f"  鎵规[?]澶[?]皬: {train_config['batch']}")
    ...
    try:
        ...
        print("\n璁[?]粌瀹屾垚!")
        print(f"最佳权重: {train_config['project']}/{train_config['name']}/weights/best.pt")
